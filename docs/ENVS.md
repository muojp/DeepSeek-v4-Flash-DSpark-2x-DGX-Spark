...
| `DSPARK_TOKENTRACE_EXPERTS` | **Not** `VLLM_*`. Default `0`. `1` applies `patches/hotfix-dsv4-tokentrace-experts.py` at boot: the V2 GPU model runner logs the top-k expert ids of every token for all 43 MoE layers, per engine step, to `DSPARK_TOKENTRACE_DIR` (default `/cache/huggingface/tokentrace`, i.e. host `~/.cache/huggingface/tokentrace`) on each node — see [TOKENTRACE.md](TOKENTRACE.md). `--enable-return-routed-experts` is **not** usable here: DSpark exists only in the V2 runner and V2 rejects that flag. Boot-time only. |
